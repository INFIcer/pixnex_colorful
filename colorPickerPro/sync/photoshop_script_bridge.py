...
Port of colorink's ``core.photoshop_script_bridge`` (GPL-3.0).

Green editions register no COM automation interface, so COM-based sync
cannot attach to them.  Instead we deploy a **hidden CEP background
extension** into ``<ps_dir>/Required/CEP/extensions/``.  The panel's JS
runs in a persistent CEF runtime with Node.js enabled, polls the bridge
files every 0.1 s and applies colors via ``evalScript``.
"""
...
EXTENSION_ID: Final = "com.pixnex.bridge"
EXTENSION_DIR_NAME: Final = "PixnexBridge"
...
<ExtensionManifest xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" ExtensionBundleId="{ext_id}" ExtensionBundleName="PixnexBridge" ExtensionBundleVersion="1.0.0" Version="7.0">
  <ExtensionList>
